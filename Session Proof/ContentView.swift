                        ...
                        do {
                            ...
                            if let user = authService.currentUser,
                               ...
                               user.subscriptionStatus == "expired" {
                                // Only convert if trial has actually ended (not brand new users with status "free")
                                if let trialEnded = user.trialEndsAt, trialEnded < Date() {
                                    // Check if they have any owned projects
                                    let projectCount = try? await authService.getUserOwnedProjectsCount(userId: user.id)
                                    if projectCount == 0 {
                                        print("🔄 Auto-converting expired producer with no projects to approver")
                                        try? await authService.convertToApprover()
                                        print("✅ Auto-converted to approver (trial expired, no owned projects)")
                                    }
                                }
                            }
